...
import { SeoilConfig } from '../config/seoil.config';
...
@Injectable()
export class SeoilCalculator implements Calculator {
    private readonly type = CalculatorEnum.SEOIL;
    private readonly config = new SeoilConfig();
    ...
    getAdmissionMapper(): Record<string, string> {
        return SeoilConfig.ADMISSION_CODE_TO_NAME;
    }

    getUnitMapper(): Record<string, string> {
        return SeoilConfig.UNIT_CODE_TO_NAME;
    }
}
