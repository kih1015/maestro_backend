...
import { YuhanConfig } from '../config/yuhan.config';
...
@Injectable()
export class YuhanCalculator implements Calculator {
    private readonly type = CalculatorEnum.YUHAN;
    private readonly config = new YuhanConfig();
    ...
    getAdmissionMapper(): Record<string, string> {
        return YuhanConfig.ADMISSION_CODE_TO_NAME;
    }

    getUnitMapper(): Record<string, string> {
        return YuhanConfig.UNIT_CODE_TO_NAME;
    }
}
