...
import { SamyookHealthConfig } from '../config/samyook-health.config';
...
@Injectable()
export class SamyookHealthCalculator implements Calculator {
    private readonly type = CalculatorEnum.SAMYOOK_HEALTH;
    private readonly config = new SamyookHealthConfig();
    ...
    getAdmissionMapper(): Record<string, string> {
        return SamyookHealthConfig.ADMISSION_CODE_TO_NAME;
    }

    getUnitMapper(): Record<string, string> {
        return SamyookHealthConfig.UNIT_CODE_TO_NAME;
    }
}
